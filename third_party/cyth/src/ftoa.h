...
#define FTOA_FLOAT32_MANTBITS 23
#define FTOA_FLOAT32_EXPBITS 8
#define FTOA_FLOAT32_BIAS (-127)

#define FTOA_FLOAT64_MANTBITS 52
#define FTOA_FLOAT64_EXPBITS 11
#define FTOA_FLOAT64_BIAS (-1023)

#define FTOA_POW10_MIN (-348)
#define FTOA_POW10_MAX 347
#define FTOA_MAX_UINT64 0xFFFFFFFFFFFFFFFFULL
#define FTOA_PUSH(dst, len, cap, ch)                                                               \
  do                                                                                               \
  {                                                                                                \
    if ((len) < (cap))                                                                             \
      (dst)[(len)++] = (ch);                                                                       \
...
static const char ftoa_smalls[] = "00010203040506070809"
...
static const ftoa_uint128 ftoa_pow10_tab[696] = {
...
static inline uint64_t ftoa_float64_bits(double f)
{
  uint64_t bits;
...
static inline uint32_t ftoa_float32_bits(float f)
{
  uint32_t bits;
  memcpy(&bits, &f, sizeof(bits));
  return bits;
...
static inline int ftoa_mul_log10_2(int x)
{
  return (x * 78913) >> 18;
}

static inline int ftoa_mul_log2_10(int x)
{
  return (x * 108853) >> 15;
}

static inline int ftoa_mul_log10_2_minus_log10_4_over_3(int e)
{
  return (e * 631305 - 261663) >> 21;
}

static inline ftoa_uint128 ftoa_umul128(uint64_t x, uint64_t y)
{
  ...
  ftoa_uint128 result;
  result.hi = ac + (mid >> 32) + (ad >> 32) + (bc >> 32);
  result.lo = (mid << 32) | (uint32_t)bd;
  return result;
#endif
}

static inline ftoa_uint128 ftoa_uadd128(ftoa_uint128 u, uint64_t n)
{
  uint64_t sum = u.lo + n;
  if (sum < u.lo)
  {
...
static inline uint64_t ftoa_umul64(uint32_t x, uint32_t y)
{
  return (uint64_t)x * (uint64_t)y;
}

static inline uint64_t ftoa_umul96_upper64(uint32_t x, uint64_t y)
{
  uint32_t yh = (uint32_t)(y >> 32);
  uint32_t yl = (uint32_t)y;
  uint64_t xyh = ftoa_umul64(x, yh);
  uint64_t xyl = ftoa_umul64(x, yl);
  return xyh + (xyl >> 32);
}

static inline uint64_t ftoa_umul96_lower64(uint32_t x, uint64_t y)
{
  return (uint64_t)x * y;
}

static inline uint64_t ftoa_umul128_upper64(uint64_t x, uint64_t y)
{
  uint32_t a = (uint32_t)(x >> 32);
  uint32_t b = (uint32_t)x;
  uint32_t c = (uint32_t)(y >> 32);
  uint32_t d = (uint32_t)y;
  uint64_t ac = ftoa_umul64(a, c);
  uint64_t bc = ftoa_umul64(b, c);
  uint64_t ad = ftoa_umul64(a, d);
  uint64_t bd = ftoa_umul64(b, d);
  uint64_t intermediate = (bd >> 32) + (uint64_t)(uint32_t)ad + (uint64_t)(uint32_t)bc;
  return ac + (intermediate >> 32) + (ad >> 32) + (bc >> 32);
}

static inline ftoa_uint128 ftoa_umul192_upper128(uint64_t x, ftoa_uint128 y)
{
  ftoa_uint128 r = ftoa_umul128(x, y.hi);
  uint64_t t = ftoa_umul128_upper64(x, y.lo);
  return ftoa_uadd128(r, t);
}

static inline ftoa_uint128 ftoa_umul192_lower128(uint64_t x, ftoa_uint128 y)
{
  uint64_t high = x * y.hi;
  ftoa_uint128 high_low = ftoa_umul128(x, y.lo);
  ftoa_uint128 result;
  result.hi = high + high_low.hi;
  result.lo = high_low.lo;
  return result;
}

static inline int ftoa_pow10(int e, ftoa_uint128* mant)
{
  if (e < FTOA_POW10_MIN || e > FTOA_POW10_MAX)
  {
    return 0;
  }
  *mant = ftoa_pow10_tab[e - FTOA_POW10_MIN];
  return 1 + ftoa_mul_log2_10(e);
}

static inline void ftoa_dbox_pow64(int k, int e, ftoa_uint128* phi, int* beta)
{
  ftoa_uint128 m = { 0 };
  int e1 = ftoa_pow10(k, &m);
  if (k < 0 || k > 55)
  {
    m.lo++;
  }
...
static inline void ftoa_dbox_pow32(int k, int e, uint64_t* mant, int* exp)
{
  ftoa_uint128 m = { 0 };
  int e1 = ftoa_pow10(k, &m);
  if (k < 0 || k > 27)
...
static inline void ftoa_dbox_mul_pow64(uint64_t u, ftoa_uint128 phi, uint64_t* int_part,
                                       int* is_int)
{
  ftoa_uint128 r = ftoa_umul192_upper128(u, phi);
  *int_part = r.hi;
  *is_int = (r.lo == 0);
}

static inline void ftoa_dbox_mul_pow32(uint32_t u, uint64_t phi, uint32_t* int_part, int* is_int)
{
  uint64_t r = ftoa_umul96_upper64(u, phi);
  *int_part = (uint32_t)(r >> 32);
  *is_int = ((uint32_t)r == 0);
}

static inline void ftoa_dbox_parity64(uint64_t mant2, ftoa_uint128 phi, int beta, int* parity,
                                      int* is_int)
{
  ftoa_uint128 r = ftoa_umul192_lower128(mant2, phi);
  *parity = ((r.hi >> (64 - beta)) & 1) != 0;
  *is_int = (((r.hi << beta) | (r.lo >> (64 - beta))) == 0);
}

static inline void ftoa_dbox_parity32(uint32_t mant2, uint64_t phi, int beta, int* parity,
                                      int* is_int)
{
  uint64_t r = ftoa_umul96_lower64(mant2, phi);
  *parity = ((r >> (64 - beta)) & 1) != 0;
  *is_int = ((uint32_t)(r >> (32 - beta)) == 0);
}

static inline uint32_t ftoa_dbox_delta64(ftoa_uint128 phi, int beta)
{
  return (uint32_t)(phi.hi >> (64 - 1 - beta));
}

static inline uint32_t ftoa_dbox_delta32(uint64_t phi, int beta)
{
  return (uint32_t)(phi >> (64 - 1 - beta));
}

static inline void ftoa_dbox_range64(ftoa_uint128 phi, int beta, uint64_t* left, uint64_t* right)
{
  *left =
    (phi.hi - (phi.hi >> (FTOA_FLOAT64_MANTBITS + 2))) >> (64 - FTOA_FLOAT64_MANTBITS - 1 - beta);
  *right =
    (phi.hi + (phi.hi >> (FTOA_FLOAT64_MANTBITS + 1))) >> (64 - FTOA_FLOAT64_MANTBITS - 1 - beta);
}

static inline void ftoa_dbox_range32(uint64_t phi, int beta, uint32_t* left, uint32_t* right)
{
  *left = (uint32_t)((phi - (phi >> (FTOA_FLOAT32_MANTBITS + 2))) >>
                     (64 - FTOA_FLOAT32_MANTBITS - 1 - beta));
  *right = (uint32_t)((phi + (phi >> (FTOA_FLOAT32_MANTBITS + 1))) >>
                      (64 - FTOA_FLOAT32_MANTBITS - 1 - beta));
}

static inline uint64_t ftoa_dbox_round_up64(ftoa_uint128 phi, int beta)
{
  return ((phi.hi >> (64 - FTOA_FLOAT64_MANTBITS - 2 - beta)) + 1) / 2;
}

static inline uint32_t ftoa_dbox_round_up32(uint64_t phi, int beta)
{
  return (uint32_t)((phi >> (64 - FTOA_FLOAT32_MANTBITS - 2 - beta)) + 1) / 2;
}

static inline uint64_t ftoa_rotate_left64(uint64_t x, int k)
{
  int n = 64;
  int s = (unsigned int)k & (n - 1);
  return (x << s) | (x >> (n - s));
}

static inline void ftoa_trim_zeros(uint64_t x, uint64_t* out_x, int* out_p)
{
  const uint64_t div1e8m = 0xc767074b22e90e21ULL;
  const uint64_t div1e8le = FTOA_MAX_UINT64 / 100000000ULL;
  const uint64_t div1e4m = 0xd288ce703afb7e91ULL;
  const uint64_t div1e4le = FTOA_MAX_UINT64 / 10000ULL;
  const uint64_t div1e2m = 0x8f5c28f5c28f5c29ULL;
  const uint64_t div1e2le = FTOA_MAX_UINT64 / 100ULL;
  const uint64_t div1e1m = 0xcccccccccccccccdULL;
  const uint64_t div1e1le = FTOA_MAX_UINT64 / 10ULL;

  int p = 0;
  uint64_t d;
  for (d = ftoa_rotate_left64(x * div1e8m, -8); d <= div1e8le;
       d = ftoa_rotate_left64(x * div1e8m, -8))
  {
    x = d;
    p += 8;
  }
  d = ftoa_rotate_left64(x * div1e4m, -4);
  if (d <= div1e4le)
  {
    x = d;
    p += 4;
  }
  d = ftoa_rotate_left64(x * div1e2m, -2);
  if (d <= div1e2le)
  {
    x = d;
    p += 2;
  }
  d = ftoa_rotate_left64(x * div1e1m, -1);
  if (d <= div1e1le)
  {
    x = d;
    p += 1;
...
static inline int ftoa_format_base10(uint8_t* a, int len, uint64_t u)
{
  int i = len;
  while ((u >> 29) != 0)
  {
    uint64_t q = u / 1000000000ULL;
    uint32_t lo = (uint32_t)(u % 1000000000ULL);
    u = q;
    for (int j = 0; j < 4; j++)
    {
      ...
      lo /= 100;
      i -= 2;
      a[i + 0] = ftoa_smalls[dd + 0];
      a[i + 1] = ftoa_smalls[dd + 1];
    }
    i--;
    a[i] = ftoa_smalls[lo * 2 + 1];
    if (u == 0)
    {
      return i;
    }
  ...
  i--;
  uint32_t dd = lo * 2;
  a[i] = ftoa_smalls[dd + 1];
  if (lo >= 10)
  {
    i--;
    a[i] = ftoa_smalls[dd + 0];
  }
  return i;
}

static inline void ftoa_dbox_digits(ftoa_decimal_slice* d, uint64_t mant, int exp)
{
  uint8_t buf[32];
  int i = ftoa_format_base10(buf, sizeof(buf), mant);
  int nd = sizeof(buf) - i;
  memcpy(d->d, buf + i, nd);
  d->nd = nd;
  d->dp = nd + exp;
...
static inline void ftoa_dbox_ftoa64(ftoa_decimal_slice* d, uint64_t mant, int exp, int denorm)
{
  if (mant == (1ULL << FTOA_FLOAT64_MANTBITS) && !denorm)
  {
    int k0 = -ftoa_mul_log10_2_minus_log10_4_over_3(exp);
    ftoa_uint128 phi;
    int beta;
    ftoa_dbox_pow64(k0, exp, &phi, &beta);
    uint64_t xi, zi;
    ftoa_dbox_range64(phi, beta, &xi, &zi);
    if (exp != 2 && exp != 3)
    {
      xi++;
    }
    ...
    if (xi <= q * 10)
    {
      uint64_t trimmed;
      int zeros;
      ftoa_trim_zeros(q, &trimmed, &zeros);
      ftoa_dbox_digits(d, trimmed, -k0 + 1 + zeros);
      return;
    }
    uint64_t yru = ftoa_dbox_round_up64(phi, beta);
    if (exp == -77 && yru % 2 != 0)
    {
      yru--;
    }
    ...
    ftoa_dbox_digits(d, yru, -k0);
    return;
  }

  const int kappa = 2;
  const uint32_t p10k = 100;
  const uint32_t p10k1 = 1000;

  int k0 = -ftoa_mul_log10_2(exp);
  ftoa_uint128 phi;
  int beta;
  ftoa_dbox_pow64(kappa + k0, exp, &phi, &beta);
  uint64_t zi_val;
  int exact;
  ftoa_dbox_mul_pow64((uint64_t)(mant * 2 + 1) << beta, phi, &zi_val, &exact);
  uint64_t s = zi_val / p10k1;
  uint32_t r = (uint32_t)(zi_val % p10k1);
  uint32_t delta_i = ftoa_dbox_delta64(phi, beta);
  ...
  if (r < delta_i)
  {
    if (r != 0 || !exact || mant % 2 == 0)
    {
      uint64_t trimmed;
      int zeros;
      ftoa_trim_zeros(s, &trimmed, &zeros);
      ftoa_dbox_digits(d, trimmed, -k0 + 1 + zeros);
      return;
    }
    s--;
  ...
  {
    int parity, exact2;
    ftoa_dbox_parity64(mant * 2 - 1, phi, beta, &parity, &exact2);
    if (parity || (exact2 && mant % 2 == 0))
    {
      uint64_t trimmed;
      int zeros;
      ftoa_trim_zeros(s, &trimmed, &zeros);
  ...
  {
    int parity, exact2;
    ftoa_dbox_parity64(mant * 2, phi, beta, &parity, &exact2);
    if (parity != ((D - p10k / 2) % 2 != 0) || (exact2 && yru % 2 != 0))
    {
      yru--;
    }
  }
...
static inline void ftoa_dbox_ftoa32(ftoa_decimal_slice* d, uint32_t mant, int exp, int denorm)
{
  if (mant == (1U << FTOA_FLOAT32_MANTBITS) && !denorm)
  {
    int k0 = -ftoa_mul_log10_2_minus_log10_4_over_3(exp);
    uint64_t phi;
    int beta;
    ftoa_dbox_pow32(k0, exp, &phi, &beta);
    uint32_t xi, zi;
    ftoa_dbox_range32(phi, beta, &xi, &zi);
    if (exp != 2 && exp != 3)
    {
      xi++;
    }
    ...
    if (xi <= q * 10)
    {
      uint64_t trimmed;
      int zeros;
      ftoa_trim_zeros((uint64_t)q, &trimmed, &zeros);
      ftoa_dbox_digits(d, trimmed, -k0 + 1 + zeros);
      return;
    }
    uint32_t yru = ftoa_dbox_round_up32(phi, beta);
    if (exp == -77 && yru % 2 != 0)
    {
      yru--;
    }
    ...
    ftoa_dbox_digits(d, (uint64_t)yru, -k0);
    return;
  }

  const int kappa = 1;
  const uint32_t p10k = 10;
  const uint32_t p10k1 = 100;

  int k0 = -ftoa_mul_log10_2(exp);
  uint64_t phi;
  int beta;
  ftoa_dbox_pow32(kappa + k0, exp, &phi, &beta);
  uint32_t zi_val;
  int exact;
  ftoa_dbox_mul_pow32((mant * 2 + 1) << beta, phi, &zi_val, &exact);
  uint32_t s = zi_val / p10k1;
  uint32_t r = zi_val % p10k1;
  uint32_t delta_i = ftoa_dbox_delta32(phi, beta);
  ...
  if (r < delta_i)
  {
    if (r != 0 || !exact || mant % 2 == 0)
    {
      uint64_t trimmed;
      int zeros;
      ftoa_trim_zeros((uint64_t)s, &trimmed, &zeros);
      ftoa_dbox_digits(d, trimmed, -k0 + 1 + zeros);
      return;
    }
    s--;
  ...
  {
    int parity, exact2;
    ftoa_dbox_parity32(mant * 2 - 1, phi, beta, &parity, &exact2);
    if (parity || (exact2 && mant % 2 == 0))
    {
      uint64_t trimmed;
      int zeros;
  ...
  {
    int parity, exact2;
    ftoa_dbox_parity32(mant * 2, phi, beta, &parity, &exact2);
    if (parity != ((D - p10k / 2) % 2 != 0) || (exact2 && yru % 2 != 0))
    {
      yru--;
    }
...
static inline void ftoa_dbox_ftoa(ftoa_decimal_slice* d, uint64_t mant, int exp, int denorm,
                                  int bit_size)
{
  if (bit_size == 32)
  {
    ftoa_dbox_ftoa32(d, (uint32_t)mant, exp, denorm);
    return;
  }
  ftoa_dbox_ftoa64(d, mant, exp, denorm);
}

static inline int ftoa_format_digits(uint8_t* dst, int cap, int neg, ftoa_decimal_slice* d,
                                     int prec)
{
  int len = 0;
  if (neg)
  ...
  if (d->dp > 0)
  {
    int m = d->nd < d->dp ? d->nd : d->dp;
    for (int i = 0; i < m; i++)
    {
      FTOA_PUSH(dst, len, cap, d->d[i]);
    }
    for (int i = m; i < d->dp; i++)
    {
...
static inline int ftoa_generic_ftoa(uint8_t* dst, int cap, double val, int bit_size)
{
  ...
  if (bit_size == 32)
  {
    bits = (uint64_t)ftoa_float32_bits((float)val);
    mantbits = FTOA_FLOAT32_MANTBITS;
    expbits = FTOA_FLOAT32_EXPBITS;
    bias = FTOA_FLOAT32_BIAS;
  }
  else
  {
    bits = ftoa_float64_bits(val);
    mantbits = FTOA_FLOAT64_MANTBITS;
    expbits = FTOA_FLOAT64_EXPBITS;
    bias = FTOA_FLOAT64_BIAS;
  }

  int neg = (bits >> (expbits + mantbits)) != 0;
  int exp = (int)(bits >> mantbits) & ((1 << expbits) - 1);
  uint64_t mant = bits & ((1ULL << mantbits) - 1);
  int denorm = 0;
  ...
  {
    int prec = 0;
    return ftoa_format_digits(dst, cap, neg, &digs, prec);
  }
...
static inline int ftoa(uint8_t* dst, int cap, double val)
{
  const int bit_size = ((val < 0.0 ? -val : val) < 16777216.0) ? 32 : 64;
  return ftoa_generic_ftoa(dst, cap, val, bit_size);
}

#undef FTOA_FLOAT32_MANTBITS
#undef FTOA_FLOAT32_EXPBITS
#undef FTOA_FLOAT32_BIAS
#undef FTOA_FLOAT64_MANTBITS
#undef FTOA_FLOAT64_EXPBITS
#undef FTOA_FLOAT64_BIAS
#undef FTOA_POW10_MIN
#undef FTOA_POW10_MAX
#undef FTOA_MAX_UINT64
#undef FTOA_PUSH
#endif
